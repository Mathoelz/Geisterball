...
ShaderLightMapper::ShaderLightMapper()
{
	...
	BaseShader Shader;
	bool loaded = Shader.load(ASSET_DIRECTORY"Shader/Vertex/vslightdummy.glsl", ASSET_DIRECTORY"Shader/Fragment/fslightdummy.glsl");
	assert(loaded);
	...
	int CPUSize = sizeof(ShaderLightBlock);
	int GPUSize = BlockSize;
